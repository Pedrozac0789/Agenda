...
async function buscarDadosHoje(data) {
  try {
    ...
    ul.innerHTML = "";
    dados.forEach((item) => {
      ul.innerHTML += `<li data-id="${item.idtarefas}">
  ...
  for (let i = 0; i < maxLen; i++) {
    ...
    semanaComTarefas.forEach((dia) => {
      ...
      if (tarefa) {
        ...
        actions.innerHTML = `
          <button class="btns" title="Editar" aria-label="Editar" onclick="abrirModalEditar(${tarefa.idtarefas})"><i class='bx bx-edit'></i></button>
